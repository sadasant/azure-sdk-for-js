...
describe("CryptographyClient (all decrypts happen remotely)", () => {
  ...
  it.only("sign and verify with RS256", async function (): Promise<void> {
    // async function iteration(i: number, signatureValue: string) {
    ...
    // const secondTotal = 50;
    // const firstTotal = baseTotal - secondTotal;
    let count = 0;
    console.log(`${process.memoryUsage().heapUsed} - ${count}`);
    for (let i = 0; i < baseTotal; i++) {
      let a = new CryptographyClient(keyVaultKey.id!, credential, {
        keepAliveOptions: {
          enable: false
        }
      });
    ...
    for (let i = 0; i < baseTotal; i++) {
      let a = new CryptographyClient(keyVaultKey.id!, credential);
      count += a && 1;
      // await iteration(i, this.test!.title);
    }
    console.log(`${process.memoryUsage().heapUsed} - ${count}`);
  });
